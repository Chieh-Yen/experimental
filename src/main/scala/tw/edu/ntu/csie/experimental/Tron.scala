...
class Tron(val function : TronFunction)
{
	...
	def tron(prob : Problem, param : Parameter, eps : Double) : Vector =
	{
		...
		val lcStartTime = new Date().getTime()
		breakable {
		while(iter <= ITERATIONS && search == 1)
		{
			...
			var cDifference = new Date().getTime() - lcStartTime;
			println("Accumulated Elapsed seconds: " + cDifference * 0.001)
		}
		}
		var cDifference = new Date().getTime() - lcStartTime;
		println("Accumulated Elapsed seconds: " + cDifference * 0.001)
		println("iter final act %5.3e pre %5.3e delta %5.3e f %5.3e |g| %5.3e".format(actred, prered, delta, f, gnorm))
		w
	}
}
